...
public final class Queryutils {
    ...
    /**
     * Return a list of {@link eqClass} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<eqClass> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<eqClass> earthquakes = new ArrayList<>();
        // Try to parse the SAMPLE_JSON_RESPONSE. If there's a problem with the way the JSON
        // is formatted, a JSONException exception object will be thrown.
        // Catch the exception so the app doesn't crash, and print the error message to the logs.
        try {
            JSONObject root=new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray eqreport=root.getJSONArray("features");
            for(int i=0;i<eqreport.length();i++)
            {
                JSONObject feature=eqreport.getJSONObject(i);
                ...
                earthquakes.add(new eqClass(mag,place,time,url));
            }
